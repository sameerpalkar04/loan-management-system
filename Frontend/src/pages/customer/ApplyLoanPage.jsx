...
export default function ApplyLoanPage() {
  ...
  const submit = async (event) => { event.preventDefault(); if (!panCardImage) { setError("Please upload your PAN card image."); return; } setSaving(true); setError(""); try { await createLoanApplication({ loanTypeId: Number(form.loanTypeId), requestedAmount: Number(form.requestedAmount), requestedTenureMonths: Number(form.requestedTenureMonths), valuation: Number(form.valuation) }, panCardImage); navigate("/customer/applications", { state: { notice: "Your application was submitted for review." } }); } catch (requestError) { setError(requestError.message); } finally { setSaving(false); } };
  return <main className="customer-workspace"><header className="workspace-header"><Logo /><Link className="back-link" to="/customer/loan-types">← Back to loan types</Link></header><div className="application-page"><div><p className="eyebrow">NEW APPLICATION</p><h1>Tell us about the loan you need.</h1><p>Your profile is already attached. An officer will review this application after you submit it.</p></div><div className="application-layout"><form className="application-form" onSubmit={submit}><label>Loan type<select name="loanTypeId" value={form.loanTypeId} onChange={update} required>{loanTypes.map((loan) => <option value={loan.loanTypeId} key={loan.loanTypeId}>{loan.loanName} · {loan.baseInterestRate}% p.a.</option>)}</select></label><div className="form-row"><label>Requested amount<input name="requestedAmount" type="number" min="1" value={form.requestedAmount} onChange={update} placeholder="e.g. 500000" required /></label><label>Tenure in months<input name="requestedTenureMonths" type="number" min="1" value={form.requestedTenureMonths} onChange={update} placeholder="e.g. 48" required /></label></div><label>Asset valuation<input name="valuation" type="number" min="1" value={form.valuation} onChange={update} placeholder="e.g. 650000" required /></label><label>PAN card image<input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => setPanCardImage(event.target.files?.[0] || null)} required /></label>{error && <p className="form-error">{error}</p>}<button className="button button--primary" disabled={saving}>{saving ? "Submitting…" : "Submit application"}</button></form><aside className="application-summary"><span>Selected product</span><h2>{selected?.loanName || "Choose a loan type"}</h2><p>{selected?.description}</p><div><span>Interest rate</span><b>{selected?.baseInterestRate || "—"}% p.a.</b></div><div><span>Maximum amount</span><b>₹{selected?.maximumLoanAmount?.toLocaleString("en-IN") || "—"}</b></div><div><span>Maximum tenure</span><b>{selected?.maximumTenureMonths || "—"} months</b></div></aside></div></div></main>;
}
